...
- [App Store — Reducer, Context and Refresh Scheduling](store.md) - The AppProvider owns all state via useReducer+Context, exposes AppActions (including sellAsset), and runs the price refresh flow with per-symbol provider dispatch, sold-out skip-quote and history-as-of logic, series-freshness reuse, allSettled error handling, an interval timer and concurrency guards.
