...
import type { AssetSnapshot, PortfolioSnapshot } from './metrics'
...
export function assetQuantity(asset: Asset, asOf?: ISODate): number {
  return lotsUpTo(asset, asOf).reduce((sum, l) => sum + l.quantity, 0)
}
...
export function portfolioSnapshot(
  ...
  const totalChangeAbs = assets.reduce((sum, a) => sum + a.totalChangeAbs, 0)
